...
pub fn Account() -> impl IntoView {
    ...
    view! {
        <div class="flex flex-col gap-8 sm:gap-12 fade-in">
            // --- HEADER IDENTITY ---
            <section class="relative pt-4 sm:pt-6">
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                    <div class="relative group">
                        ...
                        <div class="relative h-16 w-16 rounded-full bg-[#050505] ring-2 ring-white/10 flex items-center justify-center overflow-hidden">
                             <div inner_html=move || ss58_identicon_svg(&address.get(), 64) />
                        </div>
                    </div>

                    <div class="flex flex-col gap-1">
                        <h1 class="text-xs font-bold uppercase tracking-widest text-emerald-500 mb-1">
                            "Account Details"
                        </h1>

                        <button
                            class="group flex items-center gap-3 text-left transition-all active:scale-[0.98]"
                            on:click=copy_to_clipboard
                            title="Click to copy"
                        >
                            <span class="text-xl sm:text-3xl font-mono font-bold text-white break-all hover:text-emerald-50 transition-colors">
                                { move || address.get() }
                            </span>
                            <div class="flex h-8 w-8 items-center justify-center rounded-full bg-white/5 text-neutral-400 opacity-0 group-hover:opacity-100 transition-opacity">
